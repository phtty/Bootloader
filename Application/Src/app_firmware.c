...
/*
 * TODO: 替换为 STM32H7 硬件 CRC (CRC peripheral)
 *
 * 使用 __HAL_RCC_CRC_CLK_ENABLE() 使能时钟后，
 * 调用 HAL_CRC_Calculate() 即可完成硬件 CRC32 计算。
 * 当前为软件查表实现，后续切换后删除本段。
 */
static uint32_t crc32_sw(uint32_t crc, const uint8_t *buf, size_t len)
{
    static const uint32_t table[256] = {
        ...
        0xD5B88683, 0xD1799B34, 0xDC5ABDED, 0xD89BA05A,
